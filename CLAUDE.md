...
## 핵심 설계 ⑥ — 대화 최소 조건 (대화 유도)

챗봇과 대화 없이 문제만 풀고 넘어가는 것을 막는다. 단, 횟수 강제로 인한
형식적·성의 없는 입력을 피하기 위해 "세션 단위 최소치 + 막힐 때 유도" 방식으로 설계한다.
대화 로그가 CT 측정의 원천이므로, 충분하되 질 좋은 대화를 확보하는 것이 목적이다.

### 대안 1 — 세션 단위 최소치 (문제당 강제 아님)
- 문제마다 강제하지 않고, **세션 전체에서 의미 있는 대화 최소 5~6회**를 요구한다.
- 미달 시 세션 종료(또는 평가 단계 진입)를 막고 "조금 더 대화해볼까요?" 안내.
- 어려운 문제엔 더 묻고 쉬운 문제는 넘어가는 자연스러운 대화 패턴을 허용한다.
- **품질 게이트**: 한 문장 미만의 성의 없는 단답("ㅇㅇ", "모름" 등)은 최소치 카운트에서 제외한다(최소 길이/내용 조건).

### 대안 2 — 막힐 때 챗봇이 먼저 유도
- 트리거: ① 문제를 **틀렸을 때**, ② 일정 시간(예: 30~60초) 입력이 없을 때.
- 챗봇이 먼저 말을 걸어 유도 질문을 던진다(예: "이 변수가 어떻게 바뀌는지 같이 볼까요?").
- 강제가 아니라 권유이므로 거부감이 적고, 자연스럽게 최소치 달성을 돕는다.
- 유도 질문도 소크라테스 원칙을 지킨다(정답 미제공).

### 동작 요약
```
세션당 최소 5~6회(품질 조건 충족분만 카운트)
+ 틀린 문제 / 일정 시간 무입력 시 챗봇이 먼저 유도
→ 평균적으로 문제당 1~2회 대화가 자연스럽게 발생
```
권장 기본값(세션 6회, 무입력 45초, 최소 1문장)은 조정 가능한 파라미터로 둔다.
...
### Task 10 — 대화 최소 조건 + 막힐 때 유도 (핵심 설계 ⑥)
- [대안 1] 세션 단위 대화 카운터 구현. 의미 있는 대화 최소 5~6회 미달 시 세션 종료/평가 진입을 막고 안내.
  - 품질 게이트: 한 문장 미만 단답은 카운트 제외(최소 길이/내용 조건).
- [대안 2] 챗봇 선제 유도 트리거 구현: ① 문제 오답 시 ② 일정 시간(기본 45초) 무입력 시 챗봇이 먼저 유도 질문(소크라테스 원칙 유지).
- 임계값(세션 6회, 무입력 45초, 최소 1문장)은 설정값으로 분리해 조정 가능하게 둔다.
- 프론트엔드: 대화 횟수 안내 표시, 무입력 타이머 처리.
...
13. **대화 최소치는 횟수가 아니라 질이 목적.** 성의 없는 단답은 카운트 제외, 강제보다 유도를 우선.
...
- [ ] Task 8: rag.py 청크 크기 분리 (선택)
- [ ] Task 9: FAISS 재빌드 + 동작 확인
- [ ] Task 10: 대화 최소 조건 + 막힐 때 유도 (세션 6회 + 무입력 유도)
